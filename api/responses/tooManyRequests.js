...
module.exports = function tooManyRequests( optionalData ) {
	...
	var statusCodeToSet = 429;

	// If no data was provided, use res.sendStatus().
	if ( optionalData === undefined ) {
		sails.log.info( 'Ran custom response: res.tooManyRequests()' );
	...
	// except in production use res.sendStatus().
		...
		sails.log.info( 'Custom response `res.tooManyRequests()` called with an Error:', optionalData );
